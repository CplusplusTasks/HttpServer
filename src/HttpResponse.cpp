...
namespace network {
    static string getReason(int code) {
        switch (code) {
            ...
            default:
                throw HttpResponseException("unknown status code: " + std::to_string(code));
        }
    }
...
HttpResponseException::HttpResponseException(string msg) :
        runtime_error(msg) {
}
...
HttpResponse &HttpResponse::send(network::TcpSocket *socket) {
